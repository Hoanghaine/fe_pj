...
function changeImage() {
	...
	if (image) {
		const imageSources = [
			"../fonts/img/SLIDER1.jpg",
			"../fonts/img/banner-naciva-270622.jpg",
		];
		let currentImageIndex =
			parseInt(image.getAttribute("data-image-index")) || 0;
...
function sortProductByPrice() {
	...
	const products = document.querySelectorAll(
		".section_Hotproducts_content ul li"
	);
		...
		if (selectedValue === "thapcao") {
			products.forEach((product, index) => {
				const price = parseFloat(
					product.querySelector(".section_Hotproducts_content-price p")
						.textContent
				);
...
function changeImageProduct(direction) {
	...
	if (direction === "left" && currentImageIndex === 1) {
		...
		const chaild = document.getElementsByClassName(
			`chaild_img-${currentImageIndex}`
		)[0];
